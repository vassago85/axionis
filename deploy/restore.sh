#!/usr/bin/env bash
# Restore an Axionis POS deployment from a backup.sh-produced .db file.
#
# Usage:
#   ./restore.sh /opt/axionis /opt/axionis/backups/db-20260428-023001.db
#
# This script:
...
ROOT="${1:?client root required, e.g. /opt/axionis}"
BACKUP="${2:?backup .db file required}"
NAME="$(basename "$ROOT")"
DB_FILE="$ROOT/data/axionis.db"
