...
public static class DbSeeder
{
    public static async Task SeedAsync(IServiceProvider sp, CancellationToken ct = default)
    {
        ...
        var db = scope.ServiceProvider.GetRequiredService<AxionisDbContext>();
        ...
        var seedDemo = Environment.GetEnvironmentVariable("SEED_DEMO_DATA");
    ...
    private static async Task EnsurePricingColumnsAsync(AxionisDbContext db, CancellationToken ct)
    {
    ...
    private static async Task EnsureProductColumnsAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
    ...
    private static async Task EnsureAspNetUsersSupplierColumnAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        try { await db.Database.ExecuteSqlRawAsync(
    ...
    private static async Task EnsureSupplierColumnsAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        try { await db.Database.ExecuteSqlRawAsync(
    ...
    private static async Task EnsureInvoiceVoidColumnsAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        try { await db.Database.ExecuteSqlRawAsync(
    ...
    private static async Task EnsureInvoiceLineColumnsAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        try { await db.Database.ExecuteSqlRawAsync(
    ...
    private static async Task EnsureStockReceiptsTableAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
    ...
    private static async Task EnsurePromotionsTablesAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        await db.Database.ExecuteSqlRawAsync("""
    ...
    private static async Task EnsureInvoiceDiscountColumnsAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
    ...
    private static async Task EnsureStockReceiptCostColumnAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        try { await db.Database.ExecuteSqlRawAsync("""ALTER TABLE "StockReceipts" ADD COLUMN "CostPrice" TEXT;""", ct); } catch { }
    }

    private static async Task EnsureCustomersTableAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        await db.Database.ExecuteSqlRawAsync("""
    ...
    private static async Task EnsureInvoiceBusinessColumnsAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
    ...
    /// </summary>
    private static async Task EnsureCustomerAccountColumnsAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
    ...
    /// All existing invoices are backfilled so AmountPaid = GrandTotal and PaymentStatus = 'Paid'.
    /// </summary>
    private static async Task EnsureInvoiceAccountColumnsAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        ...
        // One-shot backfill: any invoice whose AmountPaid is still the default is treated as fully paid.
        try { await db.Database.ExecuteSqlRawAsync(
    ...
    /// <summary>Phase 3B.1 — adds the AccountsEnabled feature flag (default false).</summary>
    private static async Task EnsureBusinessSettingsAccountsToggleAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
    ...
    /// <summary>Phase 3B.1 — creates the CustomerPayments table for AR receipts.</summary>
    private static async Task EnsureCustomerPaymentsTableAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        await db.Database.ExecuteSqlRawAsync("""
    ...
    private static async Task EnsureConsignmentBatchesTablesAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        await db.Database.ExecuteSqlRawAsync("""
    ...
    private static async Task EnsureConsignmentBatchColumnsAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        try { await db.Database.ExecuteSqlRawAsync(
    ...
    private static async Task EnsureSpecialOrderColumnsAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
    ...
    private static async Task MergeDuplicateSkusAsync(AxionisDbContext db, ILogger log, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
    ...
    private static async Task EnsureProductPricingColumnsAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
    ...
    /// </summary>
    private static async Task EnsureImportPresetsSupplierNullableAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        try
        {
    ...
    private static async Task EnsurePricingRulesTableAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        await db.Database.ExecuteSqlRawAsync(
            """
    ...
    private static async Task EnsureQuotesTablesAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        await db.Database.ExecuteSqlRawAsync(
            """
    ...
    private static async Task EnsureBusinessSettingsTableAsync(AxionisDbContext db, CancellationToken ct)
    {
        if (!db.Database.IsSqlite()) return;
        await db.Database.ExecuteSqlRawAsync(
            """
    ...
    private static async Task EnsureMailSettingsTableAsync(AxionisDbContext db, CancellationToken ct)
    {
