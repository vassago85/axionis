...
{
    private readonly AxionisDbContext _db;

    public ImportService(AxionisDbContext db) => _db = db;

    public async Task<(List<ImportPreviewRowDto> Rows, List<string> Warnings)> PreviewWorkbookSheetAsync(
    ...
    /// <summary>Workbook import from CSV: same auto column mapping as Excel; first row must be headers.</summary>
    public async Task<(List<ImportPreviewRowDto> Rows, List<string> Warnings)> PreviewWorkbookCsvAsync(
    ...
    public async Task<int> CommitWorkbookPreviewAsync(List<ImportPreviewRowDto> validRows, Guid? supplierId, CancellationToken ct)
    {
    ...
    private static HeaderMap AutoMapHeaders(Dictionary<int, string> headers, IXLWorksheet? sheetForMerge)
    {
        ...
        // Distributor sheets: cost = wholesale ex VAT ("Price Excl."). List retail = ex VAT × 1.5; POS sell = rounded (typo "Huntes" in some files).
        ...
        // Ex-VAT wholesale only — skip distributor-specific columns (avoid "Huntex sale p"). "Price Excl." before "Price Excluding x 1.5" in column order.
        var cost = Match("price excl", "excl.", "ex vat", "exclusive")
                   ?? Match("cost", "buy", "wholesale", "unit cost", "net");
        // Sell: prefer rounded distributor column; then list retail inc VAT; skip unrounded / raw ×1.5 / line wholesale inc.
        int? sell = Match(
            "huntes rounded",
            "huntex rounded",
            "rounded to next 10",
        ...
        if (sheetForMerge != null && !IsExplicitRoundedColumn(headers, sell))
            sell = TryResolveRoundedSellFromMerge(sheetForMerge, headers) ?? sell;
    ...
    /// <summary>Header already names the distributor rounded column (e.g. "Huntes Rounded to next 10").</summary>
    private static bool IsExplicitRoundedColumn(Dictionary<int, string> headers, int? sellCol)
    {
    ...
    /// <summary>
    /// Second data column under a row-1 distributor group merge (rounded price), or column after a lone distributor label.
    /// Skips headers that are distributor sale / unrounded.
    /// </summary>
    private static int? TryResolveRoundedSellFromMerge(IXLWorksheet sheet, Dictionary<int, string> headers)
    {
    ...
    private static decimal ParseDecimal(string s, decimal fallback) =>
        ImportNumberParsing.ParseAmount(s, fallback);
