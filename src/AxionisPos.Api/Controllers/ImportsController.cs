...
public class ImportsController : ControllerBase
{
    ...
    public ImportsController(ImportService import, AxionisDbContext db)
    {
        _import = import;
        _db = db;
    }
    ...
    public async Task<ActionResult<object>> ImportWorkbook(
        ...
        CancellationToken ct = default)
    {
        ...
        var (rows, warnings) = ext == ".csv"
            ? await _import.PreviewWorkbookCsvAsync(stream, supplierId, ct)
            : await _import.PreviewWorkbookSheetAsync(stream, sheetName, supplierId, ct);
        if (!commit)
            return Ok(new { preview = rows, warnings });

        var valid = rows.Where(r => r.Error == null).ToList();
        var n = await _import.CommitWorkbookPreviewAsync(valid, supplierId, ct);
