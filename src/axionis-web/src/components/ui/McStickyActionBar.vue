...
<template>
  <div
    class="ax-sticky-bar"
    :class="[
      `ax-sticky-bar--${position ?? 'bottom'}`,
      { 'ax-sticky-bar--bare': bare }
    ]"
  >
    <div class="ax-sticky-bar__inner">
      <slot />
    </div>
  </div>
</template>
...
.ax-sticky-bar {
  ...
  box-shadow: var(--ax-app-shadow-sm, 0 2px 8px rgba(0,0,0,0.08));
}
.ax-sticky-bar--bottom {
  bottom: 0;
...
@media (max-width: 640px) {
  ...
  .ax-sticky-bar__inner > * {
    flex: 1 1 45%;
    min-height: var(--ax-touch-min, 44px);
  }
}
</style>
