...
const LEGACY_STORAGE_KEY = 'ax-branding-cache-v1'
...
function applyCssVariables(b: Branding) {
  ...
  if (b.accentColor) {
    root.style.setProperty('--ax-accent', b.accentColor)
    root.style.setProperty('--ax-focus', b.accentColor)
  } else {
    root.style.removeProperty('--ax-accent')
    root.style.removeProperty('--ax-focus')
  }
  if (b.primaryColor) root.style.setProperty('--ax-brand-primary', b.primaryColor)
  else root.style.removeProperty('--ax-brand-primary')
  if (b.secondaryColor) root.style.setProperty('--ax-brand-secondary', b.secondaryColor)
  else root.style.removeProperty('--ax-brand-secondary')
