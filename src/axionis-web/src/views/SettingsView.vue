...
<template>
  <div class="set-page">
    ...
    <McCard title="Promotions &amp; specials">
      <p class="set-hint" style="margin-top:0">
        Create named promotions (e.g. "Summer Sale 2026") with a site-wide discount %. Add per-product specials for individual pricing. Only one promotion can be active at a time.
      </p>
    ...
    <McModal v-model="showPromoForm" :title="promoEditId ? 'Edit promotion' : 'New promotion'">
      ...
      <McField label="Name" for-id="pf-name">
        <input id="pf-name" v-model="promoForm.name" required placeholder="e.g. Summer Sale 2026" />
      </McField>
      ...
      <Transition name="promo-drawer-slide">
        <aside v-if="showSpecialsDrawer" class="promo-drawer" role="dialog">
          ...
          <div class="promo-drawer__body">
            ...
            <table v-else class="ax-table" style="margin-top:1rem;font-size:0.85rem">
              <thead>
...
<style scoped>
.set-page { min-height: 100%; display: flex; flex-direction: column; gap: 1.25rem; max-width: var(--ax-container-width, 1200px); margin: 0 auto; width: 100%; }
.set-hint { margin: 0.75rem 0 0; font-size: 0.88rem; color: var(--ax-app-text-muted, #475569); line-height: 1.5; }
.set-grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; }

/* Pricing rules */
.pr-loading { display: flex; align-items: center; gap: 0.5rem; padding: 1.25rem; color: #475569; }
...
.pr-tab--active { background: #fff; color: #0f172a; box-shadow: 0 1px 2px rgba(0,0,0,0.08); }
.pr-tab__count { font-size: 0.75rem; background: #e7e5e0; padding: 0.1rem 0.45rem; border-radius: 999px; color: #475569; }
.pr-tab--active .pr-tab__count { background: var(--ax-accent, #06b6d4); color: #fff; }
.pr-hint { margin: 0.75rem 0; color: #475569; font-size: 0.9rem; }
...
.pr-row__stats dd { margin: 0; font-weight: 600; color: #0f172a; }
...
.promo-card { border: 1px solid var(--ax-app-border, #d6d3ce); border-radius: 0.5rem; padding: 0.75rem 1rem; background: var(--ax-app-surface, #fff); }
.promo-card--active { border-color: var(--ax-primary, #38a169); box-shadow: inset 3px 0 0 var(--ax-primary, #38a169); }
...
.promo-card__discount { font-size: 1.1rem; font-weight: 600; color: var(--ax-primary, #38a169); white-space: nowrap; }
.promo-card__meta { display: flex; gap: 1.5rem; margin-top: 0.25rem; font-size: 0.82rem; color: var(--ax-app-text-muted, #475569); }
...
.promo-drawer { position: fixed; top: 0; right: 0; bottom: 0; width: min(480px, 90vw); background: var(--ax-app-bg, #faf8f5); box-shadow: -4px 0 24px rgba(0,0,0,0.15); z-index: 999; display: flex; flex-direction: column; overflow: hidden; }
.promo-drawer__head { display: flex; justify-content: space-between; align-items: center; padding: 1rem 1.25rem; border-bottom: 1px solid var(--ax-app-border, #d6d3ce); }
.promo-drawer__title { font-size: 1.1rem; margin: 0; }
.promo-drawer__close { display: inline-flex; align-items: center; justify-content: center; background: none; border: none; font-size: 1.5rem; cursor: pointer; color: var(--ax-app-text-muted); }
.promo-drawer__body { flex: 1; overflow-y: auto; padding: 1rem 1.25rem; }

/* Search results dropdown */
.promo-search-results { list-style: none; margin: 0.25rem 0 0; padding: 0; border: 1px solid var(--ax-app-border, #d6d3ce); border-radius: 0.35rem; max-height: 200px; overflow-y: auto; background: var(--ax-app-surface, #fff); }
.promo-search-results li { padding: 0.5rem 0.75rem; cursor: pointer; font-size: 0.85rem; }
.promo-search-results li:hover { background: var(--ax-app-hover, #f0ede8); }
