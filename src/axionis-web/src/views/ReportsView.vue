  ...
  <div class="rep-page">
    ...
    <div v-if="showPurgeConfirm" class="rep-overlay" @click.self="showPurgeConfirm = false">
      <McCard title="Purge all transactional data?" class="rep-dialog">
        ...
        <p style="margin-top: 0.5rem; color: var(--ax-app-text-muted, #475569);">Products, wholesalers, users, and settings are not affected.</p>
...
.rep-tabs {
  ...
  border-bottom: 2px solid var(--ax-app-border-faint, #e2e8f0);
}
...
.rep-tab--active {
  ...
  border-bottom-color: var(--ax-accent, #06b6d4);
}
...
.rep-pay-card {
  ...
  flex-direction: column;
  gap: 0.25rem;
  background: var(--ax-app-surface-alt, #faf8f5);
  border: 1px solid var(--ax-app-border-soft, #cbd5e1);
  border-radius: var(--ax-app-radius-card, 18px);
...
.rep-total-row td {
  border-top: 2px solid var(--ax-app-border-subtle, #94a3b8);
  background: var(--ax-app-surface-2, #f8fafc);
}
