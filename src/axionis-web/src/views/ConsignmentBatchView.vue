...
<template>
  ...
  <template v-if="activeBatch">
    <McCard>
      ...
      <div class="cb-table-wrap">
        <table v-if="activeBatch.lines.length" class="ax-table cb-table">
          <thead>
            <tr>
              <th>SKU</th>
              <th>Product</th>
  ...
  <template v-else>
    ...
    <McCard>
      ...
      <table v-else-if="batches.length" class="ax-table">
        <thead>
          <tr>
            <th>Date</th>
            <th>Type</th>
...
.cb-progress-fill { height: 100%; background: #06b6d4; border-radius: 4px; transition: width .3s; }
...
.cb-cost-change { margin-left: .25rem; color: #06b6d4; font-weight: 700; cursor: help; }
...
.cb-cost-update { margin-top: .75rem; padding: .75rem; background: #ecfeff; border-left: 3px solid #06b6d4; border-radius: 4px; }
